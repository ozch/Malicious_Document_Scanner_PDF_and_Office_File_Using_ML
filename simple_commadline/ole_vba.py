...
if sys.version_info[0] <= 2:
    ...
    if sys.version_info[1] <= 6:
        ...
        print("Need Python 2.7 or later")
    else:
        # Python 2.7
...
class VBA_Scanner(object):
    ...
    def scan(self, include_decoded_strings=False, deobfuscate=False):
        ...
        print("--------", len(self.suspicious_keywords))
...
# MADE CHANGES HERE START ######################################################################################
        if results:
            ...
            for kw_type, keyword, description in results:
                ...
                #parsing data to count the number of suspecious keyword
                #including creating json string
                if tp=="NOTHING":
                    tp=kw_type
                    times=times+1
                elif kw_type!=tp:
                    r_number = r_number+'"{0}":"{1}"'.format(tp,times)
                    r_number = r_number+","
                    tp=kw_type
                    times=1
                elif kw_type==tp:
            ...
            r_number = r_number + '"{0}":"{1}"'.format(tp, times) + "," + '"total":"{0}"'.format(len(results)) + "}"
            #reseting tp variable for next file processing
            tp="NOTHING"
            print(t)
        else:
...
def parse_args(cmd_line_args=None):
    ...
    #hello darkness  commented whole if statement
    #if len(args) == 0:
