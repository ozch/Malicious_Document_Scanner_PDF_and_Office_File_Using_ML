...
C:\Documents and Settings\Administrator> sc create olevba binPath= "C:\Program Files\Aspera\Enterprise Server\bin\Debug\asperacentral.exe" DisplayName= "OLE VBA" start= auto
...
#this is where magic happens
		...
		if indicator == 'm' or indicator == 'M' or indicator == 'O':               		
			if ex.OUT_DIR == None or ex.OUT_DIR == '.':
				print(line)
			else:			
				s =PrintableNameC(fname, orphan)
				print '\t ::VBA OLE DATA TO BE PROCESSED'+ s.decode('utf-8')
				file = open(ex.OUT_DIR+"/"+s[1:len(s)-1]+".txt","w+") 
				p = ex.OUT_DIR.replace('./output','./feed')
				p = p.replace('_','.')
				print "\t ::OUTPUTTING TO VBA OLE DATA FROM :"+p.decode('utf-8')
				que = 'python oledump.py -s '+index+' -i '+p.decode('utf-8')
				print '\t ::GETTING -S -N -I :'+que.decode('utf-8')
...
#fucked up here
        ...
        for orphan, fname, entry_type, stream in OLEGetStreams(ole):
            if options.select == 'a' or ('%s%d' % (prefix, counter)) == options.select or prefix == 'A' and str(counter) == options.select:
                output_data = DumpFunction(DecompressFunction(DecodeFunction(decoders, options, CutData(stream, options.cut))))
                global return_data
                return_data = output_data
                #print "OUTPUT DATA : ", type(output_data),output_data
                #StdoutWriteChunked(output_data)
                if options.select != 'a':
                    break
            counter += 1
